...
import { PrintOptions } from './print-options-dialog';
...
export function PrintView({ flights, transport, notTravelling, terms, printOptions }: PrintViewProps) {
  ...
  useEffect(() => {
    ...
    if (printOptions) {
      ...
      filteredTerms = filteredTerms.filter(term => {
        if (term.school === 'benenden' && !printOptions.schools.benenden) return false;
        if (term.school === 'wycombe' && !printOptions.schools.wycombe) return false;
        return true;
      });
      
      // Update termIds after filtering
      const finalTermIds = new Set(filteredTerms.map(t => t.id));
      
      // Filter related data by termIds
      filteredFlights = flights.filter(f => finalTermIds.has(f.termId));
      filteredTransport = transport.filter(t => finalTermIds.has(t.termId));
      filteredNotTravelling = notTravelling.filter(nt => finalTermIds.has(nt.termId));
    }

    const data = transformForPrint(filteredFlights, filteredTransport, filteredNotTravelling, filteredTerms);
    setPrintData(data);
          ...
          <div className="mb-3">
            ...
            {termNotTravelling?.noFlights ? (
              ...
              <div className="space-y-2">
                {termFlights.map((flight) => (
                  <div key={flight.id} className="flight-row">
                    <div className="font-medium mb-1">
                      {flight.type === 'outbound' ? 'From School' : 'To School'} Flight
                    </div>
    ...
    <div className="print-view min-h-screen bg-white text-black p-8">
      ...
      <header className="text-center mb-8 border-b-2 border-gray-300 pb-4">
        ...
        <p className="text-sm text-gray-500 mt-2">
          {printOptions ? 
            `Academic Year ${printOptions.year} - ${getSelectedSchoolsText(printOptions)}` :
      ...
      <div className="mb-6 p-4 bg-gray-50 border">
        ...
        <div className="grid grid-cols-4 gap-4 text-sm">
          <div>
            <strong>Total Terms:</strong> {Object.values(printData).flat().length}
          </div>
          <div>
      ...
      {/* Content by School */}
      {printOptions?.layout === 'side-by-side' && Object.keys(printData).length === 2 ? (
        // Side-by-side layout for comparing both schools
        <div className="side-by-side">
          {['benenden', 'wycombe'].map(school => (
            <div key={school} className="school-column">
              <h2 className="text-lg font-bold mb-3 text-center bg-gray-100 p-2 border">
                {getSchoolDisplayName(school)}
              </h2>
              {printData[school] ? renderSchoolTerms(printData[school]) : (
